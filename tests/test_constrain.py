...
def test_constrain_initialisation():
    ...
    c = C.Constrain(position=[0, 0, 0], filter=filter, value=1.69, field_handler=fh)
    c
...
def test_constrain_correlation_lag():
    ...
    c4 = C.ThirdDerivativeConstrain(
        X4, filter=filt, value=list(range(10)), field_handler=fh
    )
...
def test_constrain_correlation_nolag():
    ...
    c4 = C.ThirdDerivativeConstrain(
        X1, filter=f4, value=list(range(10)), field_handler=fh
    )
...
def test_full_correlation():
    ...
    def test_it(use_cache):
        ...
        c3 = C.HessianConstrain(
            X3, filter=f3, value=[0, 0, 0, 0, 0, 0], field_handler=fh
        )
        fh.add_constrain(c1)
        fh.add_constrain(c2)
        fh.add_constrain(c3)
